...
<%@taglib prefix="shiro" uri="http://shiro.apache.org/tags" %> 
	<div class="tab-content panel-body pd0">
					...
					<div class="clearfix">
						<div class="control-item">
							<div class="control-group">
								<input type="hidden" value="" name="id" id="postId">
								<label class="control-label control-label-auto">组织：</label>
								<div class="control-container pull-left">
									<input id="hideOrgEmp" type="hidden" name="" value="${orgId}">
									<input id="hideNodeEmp" type="hidden" name="" value="${nodeType}">
									<!-- 下拉框选项的酒店ID -->
									...
									<span class="control-form-text nowrap input-full" id="showOptEmp"></span>
								</div>
							</div>
						</div>
						<div class="control-item">
						...
						<div class="control-item">
							<div class="control-group ml20">
								<div class="control-container">
									<input class="control-text input-w220" type="text" name="" value="" placeholder="中文名/外文名/手机号码/工号" id="inforsId" autocomplete="off">
								</div>
							</div>
						</div>
						<div class="control-item">
		...
		<!-- filter-box end -->
		<!-- grid-top-bar start -->
		<div class="grid-top-bar clearfix mt10">
			<div class="action-pop pull-left">
				<shiro:hasPermission name="employee_modify">
					<button class="button button-primary" id="btnStaffAdd" type="button" data-keyname="N">新建 (N)</button>
					<button class="button button-primary" id="btnStaffEdit" type="button" data-keyname="E" onclick="editRowEmp('')">修改 (E)</button>
				</shiro:hasPermission>
				...
				<shiro:hasPermission name="/employee/delete">
					<button class="button button-primary ml10" id="btnStaffDel" type="button" data-keyname="D" onclick="delRowEmp('')">删除 (D)</button>
				</shiro:hasPermission>
			</div>
		...
		<!-- grid start -->
		<div class="grid-content" id="gridEmp"></div>
		<!-- grid end -->
	</div>
...
BUI.use(['bui/grid', 'bui/data', 'bui/overlay', 'bui/form', 'bui/tree','bui/extensions/treepicker', 'bui/tab'], function(Grid, Data, Overlay, Form, Tree, TreePicker, Tab) {
	var StoreDpt = Data.Store;
	var orgIdVal = $('#hideOrgEmp').val();
	var nodeTypeVal = $('#hideNodeEmp').val();
  	var org = OrgTree.findNode(orgId);
  	var orgName = org.text;
  	$("#showOptEmp").text(orgName);
	/* Grid */
    ...
    columns = [
		{ title: '操作',dataIndex: 'id', sortable: false, width: 160, renderer : function(value,obj){ 
				...
				return  '<shiro:hasPermission name="employee_modify"> <a class="grid-icon grid-icon-edit btn-edit" title="编辑" href="javascript:;" onclick="editRowEmp(\''+value+'\');"></a></shiro:hasPermission>'+
				'<shiro:hasPermission name="/employee/delete"> <a class="grid-icon grid-icon-del btn-del" title="删除" href="javascript:;" onclick="delRowEmp(\''+value+'\');"></a></shiro:hasPermission>'+
		...
		{ title: '工号',dataIndex: 'staffNo', elCls: 'text-center', width: 80, showTip: true },
		{ title: '手机号码',dataIndex: 'mobilePhone', elCls: 'text-center', width: 80, showTip: true },
		{ title: '状态',dataIndex: 'status', width: 40, renderer : Grid.Format.enumRenderer({
				'2':'锁定',
				'0':'正常',
				'3':'待审核'
			})
		},
		{ title: '归属组织',dataIndex: 'orgInfo', elCls: 'text-left', width: 160, showTip: true }
	];
	...
	window.editRowEmp = function(id) {
		...
		dialogEmployee.get('loader').load({'id':id,'hotelId':currentHotelId});
	}

	//行删除
	window.delRowEmp = function(idvalue) {
	//批量删除
...
//判断是否填写数据默认为没有false
	...
	function dialogEmployeeInit(id,index) {
		dialogEmployee = new Overlay.Dialog({
			...
			loader:{
				...
				callback : function() {
					//var curr = id && index ? true : false;
					decide = false;
					oldDecide = false;
					if (id && id != '') {
						tabEmp = new Tab.TabPanel({
							render: '#tabEmp',
							elCls: 'nav-tabs',
					...
					if(index && index != 1) {
						tabEmp.setSelected(tabEmp.getItemAt(index-1));
					}
					
					tabEmp.on('selectedchange', function(ev) {
	            		var item = ev.item;
	            		var value = item.get('value');
	            		if (value != 1) {
	            			if(tabEmp.isItemSelected(item)){
									...
									if(statusA == 200){
										...
										if (accountAft == '') {
											...
											tabEmp.addChild({
												...
												value: '4',
												loader: {
													url: '<c:url value="/employee/dataRightDlog"/>',
												}
											});
										}
										/* if (!id || id == '') {
	...
	$('#btnStaffAdd').on('click', function() {
		first = 0;
		decide = false;
		dialogEmployeeInit();
		dialogEmployee.get('loader').load({'hotelId':currentHotelId});
	});
	...
	$('#btnStratery').on('click', function() {
		...
		var baseHotelId = selects[0].defaultHotelId;
