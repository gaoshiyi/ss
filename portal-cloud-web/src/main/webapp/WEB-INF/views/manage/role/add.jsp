...
<!-- 新建 Dialog start  -->
<form class="form-horizontal bui-form-horizontal bui-form bui-form-field-container" method="post" action="<c:url value="/role/saveRole"/>" autocomplete="off" data-plus-as-tab="true">
	<div class="sup-content mb20">
		...
		<div class="clearfix">
			...
			<div class="control-group pull-left ml20">
				...
				<div class="control-container box-w150 pull-left">
					<select class="choose chosen-align-right" data-placeholder="请选择集团或酒店" id="belongId2" data-rules="{required:true}" name="belongId2">
						<option value="">请选择集团或酒店</option>
						<c:forEach items="${groupHotelList }" var="groupHotel">
							<option value="${groupHotel.id }" <c:if test="${groupHotel.id eq sessionScope.SESSION_USER.hotelId}">selected</c:if>>${groupHotel.name }</option>
						</c:forEach>
					</select>
				</div>
			</div>
		...
		<div class="clearfix">
			<div class="row control-indent control-role">
				<div class="side side-left ml10">
					<div class="tree-list tree-noicon" onclick="treeClick()" id="treeList"></div>
				</div>
